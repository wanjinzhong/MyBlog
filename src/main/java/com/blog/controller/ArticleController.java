...
public class ArticleController {
	...
	public void prevAndNext(Integer id, Model model) {
		...
		if(prevArticle == null)
			prev = "<span>没有了</span>";
		else prev = "<a href='article.do?id="+ prevArticle.getArticleId() + "'>" + prevArticle.getTitle() + "</a>";
		Article nextArticle = articleService.getArticleBaseById(id + 1);
		if (nextArticle == null)
			next = "<span>没有了</span>";
		else next = "<a href='article.do?id="+ nextArticle.getArticleId() + "'>" +nextArticle.getTitle() + "</a>";
